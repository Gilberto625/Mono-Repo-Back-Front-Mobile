...
export class InventarioComponent implements OnInit {
  ...
  /** ID del producto que se está eliminando (para mostrar estado y evitar doble clic) */
  eliminandoId: number | null = null;
  /** false = Todos (historial completo), true = solo hoy */
  soloHoy = false;
  imagenesConError = new Set<number>();
  private readonly STORAGE_TAB = 'inventarioTab';
  private readonly STORAGE_SOLO_HOY = 'inventarioSoloHoy';
  ...
  /** Lista para tab Alertas: productos con alerta aplicando filtros */
  ...
  ngOnInit(): void {
    ...
    if (tab === 'stock' || tab === 'movimientos' || tab === 'alertas') {
      this.tabActivo = tab;
    }
    const hoy = sessionStorage.getItem(this.STORAGE_SOLO_HOY);
    if (hoy !== null) {
      this.soloHoy = hoy === 'true';
    }

    // Hidratar instantáneo (F5 / URL directa) desde snapshot de productos activos.
  ...
  cargarProductos(): void {
    ...
    // Solo productos activos: los eliminados (activo=false) no aparecen ni al refrescar
  ...
  exportarMovimientosCSV(): void {
    ...
    a.download = `inventario-movimientos-${scope}-${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
    URL.revokeObjectURL(a.href);
  ...
  async exportarMovimientosXLSX(): Promise<void> {
    ...
    try {
      ...
      const scope = this.soloHoy ? 'hoy' : 'historial';
      const filename = `inventario-movimientos-${scope}-${new Date().toISOString().slice(0, 10)}.xlsx`;
  ...
  /** Eliminar producto (solo permitido cuando stock es 0). Deja de aparecer en inventario. */
  eliminarProducto(producto: Producto, event?: Event): void {
    ...
    if (!confirm(`¿Eliminar "${producto.nombre}"? Ya no aparecerá en inventario.`)) return;
    this.eliminandoId = producto.id;
    this.adminService.eliminarProducto(producto.id).subscribe({
      next: () => {
        this.productos = this.productos.filter(p => p.id !== producto.id);
        this.filtrarProductos();
        this.modalService.showSuccess('Producto eliminado');
        this.eliminandoId = null;
      },
      error: (err: { error?: { error?: string }; message?: string }) => {
        this.eliminandoId = null;
        const msg = err?.error?.error || err?.message || 'No se pudo eliminar el producto';
