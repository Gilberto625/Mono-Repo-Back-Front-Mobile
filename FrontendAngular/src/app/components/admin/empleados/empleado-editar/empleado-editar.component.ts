...
export class EmpleadoEditarComponent implements OnInit {
  ...
  /** Pasado desde la lista con `[state]`; solo existe durante la navegación que crea el componente. */
  ...
  private rellenarForm(e: Empleado): void {
    if (this.esSecretaria && String(e.rol || '').toLowerCase() !== 'barbero') {
      this.modalService.showError('Secretaría solo puede editar barberos.');
      this.router.navigate([this.empleadosBase]);
      return;
    }
    ...
    this.form.rol = this.esSecretaria ? 'barbero' : (e.rol || 'barbero');
